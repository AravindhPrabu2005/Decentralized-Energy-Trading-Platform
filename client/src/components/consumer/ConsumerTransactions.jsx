...
import { ShoppingCart, CheckCircle, Filter, TrendingDown } from 'lucide-react'
...
export default function ConsumerTransactions() {
  ...
  const stats = {
    ...
    totalPaid: trades.reduce((s, t) => s + t.totalAmount, 0),
      ...
      <div className="mt-20 min-h-screen bg-gradient-to-br from-green-50 via-white to-green-50 px-4 py-8">
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">All your energy purchases</p>
          </div>

          {/* Stats */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              { label: 'Total Purchases', value: stats.total,                                   color: 'text-gray-800'  },
              { label: 'Energy Bought',   value: `${stats.totalKwh} kWh`,                      color: 'text-blue-600'  },
              { label: 'Total Spent',     value: `₹${stats.totalPaid.toLocaleString('en-IN')}`,color: 'text-red-500'   },
              { label: 'Avg Price Paid',  value: `₹${stats.avgPrice}/kWh`,                     color: 'text-green-700' },
            ].map(s => (
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center py-16 text-center space-y-3">
              ...
              <p className="text-gray-400 text-sm">Place bids in the marketplace to buy energy</p>
            </div>
          ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-100 bg-gray-50">
                      {['Date & Time', 'Energy', 'Qty (kWh)', 'Bid ₹', 'Clearing ₹', 'Total Paid', 'Auction #', 'Status'].map(h => (
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {filtered.map(trade => (
                      <tr key={trade._id} className="hover:bg-green-50/30 transition">
                        ...
                        <td className="px-5 py-4 text-gray-600">₹{trade.bidPrice}</td>
                        <td className="px-5 py-4 font-bold text-green-600">₹{trade.clearingPrice}</td>
                        <td className="px-5 py-4 font-bold text-red-500">₹{trade.totalAmount.toLocaleString('en-IN')}</td>
